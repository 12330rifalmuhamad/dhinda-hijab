...
export default function ShopTheLookSection({ section }) {
  ...
  return (
    <section className="relative w-full bg-[#FFF1F1] py-16 md:py-24 px-[5%] md:px-[10%] overflow-hidden">
      {/* Title */}
      <motion.div 
        ...
        viewport={{ once: true }}
      >
        <h2 className="text-xl md:text-2xl font-serif tracking-[0.2em] text-gray-700 uppercase">
          Shop The Look
        </h2>
      </motion.div>
      ...
      <div className="relative">
          {looks.length > 1 && (
            <>
                <button 
                    onClick={prevSlide}
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 md:-translate-x-12 z-10 p-2 text-gray-400 hover:text-gray-800 transition-colors"
                >
                    <ChevronLeft size={32} />
                </button>
                <button 
                    onClick={nextSlide}
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 md:translate-x-12 z-10 p-2 text-gray-400 hover:text-gray-800 transition-colors"
                >
