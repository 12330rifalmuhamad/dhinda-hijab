...
export default function CheckoutPage() {
  ...
  const handleCheckShipping = async () => {
    // PENTING: Di aplikasi nyata, Anda perlu fitur pencarian area untuk mendapatkan ID Area ini.
    // Untuk contoh, kita gunakan ID Area Surabaya.
    const destinationAreaId = "IDN.JI.SBY.SBY.SBY";
    ...
    try {
      ...
      const response = await fetch('/api/biteship-rates', {
        ...
        body: JSON.stringify({ destination_area_id: destinationAreaId })
      });

      if (!response.ok) {
        throw new Error('Gagal mengambil opsi pengiriman');
      }
              ...
              <div className="mt-6">
                ...
                <div className="space-y-2 mt-2">
                  {shippingOptions.map(option => (
                    <label key={option.id} htmlFor={option.id} className="flex items-center p-3 border rounded-md has-[:checked]:bg-gray-100 has-[:checked]:border-gray-900">
                      <input type="radio" name="shipping" id={option.id} onChange={() => handleShippingChange(option.price)} className="h-4 w-4" />
